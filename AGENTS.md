...
The active sequence is scale-first question-only AI exploration. The preserved v0.1 human-first lane is deferred and is not the active gate. The current path is cumulative question-only semantic-backbone recurrence/saturation analysis, a candidate backbone library and composition rules, representative environment-aware realization, and only then grounding, execution, and answer recovery:
...
- the active study plan is `question_structure_study_plan_v0_2.json`, and the current scientific decision is `UNDECIDED_NEEDS_SCALE_EVIDENCE`; no semantic instrument, operator vocabulary, executable graph, or corpus is selected, gold, universally supported, or modeling-ready merely because a schema or run exists;
...
- all 100 AI-processed IDs are recorded in the exposure ledger and are ineligible for future unseen-evaluation claims. The separate exploratory allocation leaves 3,266 official-dev questions unexposed and unallocated, while `split_manifest_v0_1.json` remains unchanged at 30 annotation-pilot and zero train/dev/locked-eval allocations;
- the next exact task is to freeze a separately versioned cumulative N=300 selection, question-only pool, and exposure manifest with the current 100 as an exact prefix and 200 newly selected eligible questions. Then generate positions 101–300 with the N=100 prompt, schema, role set, and normalizer unchanged, but route committed positions across producer partitions so question order is not confounded with one contiguous producer context, and recompute the cumulative metrics and partition sensitivity;
...
- `data_construction/exploration/ai_question_structure_scale_v0_1/`: active frozen N=100 contract, exact question-only pool, primary records, deterministic signatures, recurrence metrics, and separately versioned partition/normalization sensitivity audit.
