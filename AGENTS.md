...
The cumulative scale-first question-only AI exploration is complete through N=300. The preserved v0.1 human-first lane is deferred and is not the active gate. The current path is candidate-backbone library and representative-sampling contract freeze, representative environment-aware realization, and only then grounding, execution, and answer recovery:
...
- the next exact task is to freeze a separately versioned candidate-backbone/library-and-representative-sampling contract before reading environment or execution outcomes. Retain all 30 contracted families without post-hoc semantic merging, preserve fine/topology/task crosswalks and evidence strength, and commit deterministic frequency/rarity-stratified representative IDs before environment-aware operator realization;
...
- `data_construction/exploration/ai_question_structure_scale_v0_1/`: frozen N=100 and cumulative N=300 contracts, exact question-only pools, primary records, deterministic signatures, recurrence metrics, and partition/normalization sensitivity evidence.
...
- `data_construction/reports/research_sequencing_decision_v0_3.md`: cumulative N=300 result, precommitted no-N1000 decision, interpretation limits, and candidate-library exact next task.
