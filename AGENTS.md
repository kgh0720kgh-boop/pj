...
The cumulative scale-first question-only AI exploration, candidate-backbone library, 71-question representative environment views, first two environment-aware signals, reversible label-free normalization v0.1, and its crossed-author sensitivity diagnostic are complete and committed. The preserved v0.1 human-first lane is deferred and is not the active gate. The active step is a separately versioned normalization v0.2 revision before any provisional vocabulary, grounding, execution, or answer recovery:
...
- the next exact task is normalization v0.2 over the already exposed 93 original plus 34 crossed-author candidates. Freeze a new plan while v0.2 outputs are absent, make the semantic quotient candidate-derived with explicit eligibility/missing/output/dependency profiles, factor adapter modality/cardinality/placement/split-fuse/output components, and keep v0.1 immutable. Do not consume fresh IDs or begin grounding unless the separately frozen v0.2 criteria pass;
...
- `data_construction/exploration/ai_question_structure_scale_v0_1/operator_equivalence_crossed_author_sensitivity_v0_1/`: frozen 16-question/two-author packets, author records, normalized comparisons, checks, metrics, report, and run manifest for the active producer-confound gate.
