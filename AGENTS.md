...
The cumulative scale-first question-only AI exploration, candidate-backbone library, 71-question representative environment views, first two environment-aware signals, reversible label-free normalization v0.1, crossed-author sensitivity diagnostic, and candidate-derived normalization v0.2 are complete and committed. The preserved v0.1 human-first lane is deferred and is not the active gate. Normalization v0.2 passed its technical and factorized-adapter contracts but failed its frozen crossed-author semantic-stability criteria. The active step is a separately versioned targeted re-authoring plan over six already exposed challenge questions before any provisional vocabulary, grounding, execution, or answer recovery:
...
- reversible normalization v0.1 was committed as implementation `c372ef4b43a88b80d85f69359197bdedd3c40e61`, output-absent plan `83428b9698546de054db672e72dbae880ef4c1ac`, and materialized results `a9d3272c6cef4f411bf8f124159a634920c0571e`. All 93 candidates reconstruct losslessly; 89 are equivalent, four provisionally equivalent, and zero not equivalent. There are 39 semantic quotients and 19 environment-adapter signatures. Six of 16 multi-question families have unstable semantic sets, while fixed question/producer assignment prevents attribution;
- the normalization plan selected and completed its no-fresh-reserve fallback: two new fresh-context AI authors each covered the same frozen 16-question subset containing all seven E1 challenge cases plus nine adequate controls. Semantic-set exact match was 16/16, but v0.1's target-anchored quotient can give partial candidates the same target hash; adapter-set exact match was 0/16, and the 34 candidates contain 20 equivalent, nine provisional, and five not-equivalent results. The frozen branch is `REVISE_EQUIVALENCE_NORMALIZATION_CONTRACT`;
- normalization v0.2 was committed as implementation `16868477069f4413392c4f968c3e94c510fcb7a8`, output-absent 127-candidate plan `3d636e57b88da4469540a8ed47fa413147857060`, and materialized results `a13fa72c75049e09a966a9ea11e514c4a961841e`. All candidates reconstruct losslessly; eligibility is 121 full, one provisional-only, and five ineligible, with zero eligible-set contamination and zero unclassified adapter candidates. The technical and factorized-adapter contracts pass;
- v0.2 crossed full-eligible semantic sets match on 10/16 questions with mean Jaccard 0.65625 and 27/32 author-question coverage. Challenge/control exact matches are 1/7 and 9/9. All six mismatches are E1 challenge cases: five contain an ineligible partial author-01 candidate and one contains a provisional author-02 candidate with an extra non-target dependency. The frozen branch is `NARROW_OR_REAUTHOR_BEFORE_GROUNDING`, and grounding remains unauthorized;
- the next exact task is to freeze a separately versioned targeted re-authoring plan, before any new author output, bound to exactly `0d48bffa70ef4acf`, `cc681cfdba9badd5`, `1e2e4e4f72a64bbf`, `1e674ae4b655c1a1`, `1ca8ffcd3e20e498`, and `ba563b015b09bf21` in that order. It must use no fresh question ID, preserve every old record, isolate two new fresh-context AI authors from prior records/results and later-layer evidence, precommit all stopping branches, and forbid grounding in the run itself;
...
- `data_construction/exploration/ai_question_structure_scale_v0_1/operator_equivalence_normalization_v0_1/`: 93 label-free projections and reversible two-layer normalizations, question/family candidate sets, checks, metrics, report, and run manifest.
- `data_construction/exploration/ai_question_structure_scale_v0_1/operator_equivalence_crossed_author_sensitivity_v0_1/`: frozen 16-question/two-author packets, author records, normalized comparisons, checks, metrics, report, and run manifest for the completed producer-confound diagnostic.
- `data_construction/exploration/ai_question_structure_scale_v0_1/contracts/operator_equivalence_normalization_plan_v0_2.json`: output-absent plan binding all 127 source candidates, implementation identity, evidence exclusions, criteria, and branches.
- `data_construction/exploration/ai_question_structure_scale_v0_1/operator_equivalence_normalization_v0_2/`: 127 candidate-derived normalizations, original question/family sets, 16 crossed comparisons, checks, metrics, report, and run manifest.
...
- `data_construction/reports/research_sequencing_decision_v0_6.md`: completed normalization/crossed-author interpretation, failed frozen branch, and normalization v0.2 exact next task.
- `data_construction/reports/research_sequencing_decision_v0_7.md`: completed normalization-v0.2 interpretation, failed crossed semantic-stability criteria, and the exact six-question targeted re-authoring plan-freeze task.
...
- `tests/test_operator_equivalence_normalization.py` and `tests/test_operator_equivalence_crossed_author_sensitivity.py`: reversible quotient, adapter separation, label-free projection, frozen fallback, fresh-author packet isolation, and comparison regression tests.
- `tests/test_operator_equivalence_normalization_v0_2.py`: candidate-sensitive semantic identity, eligibility separation, factorized adapter, plan-freeze, reconstruction, and leakage regression tests.
