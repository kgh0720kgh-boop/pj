...
The project-local exact-pin `.venv` now passes full Draft 2020-12 validation for 9 schemas and 3 vocabulary instances. The pinned suite, including write-once granularity-output protection, passes `51/51` tests.

A deterministic, source-verified 30-question `annotation_schema_pilot` is allocated with `release_eligible=true`, no diagnostic override, zero overlap with the recovered 100-ID historical exclusion set, and no forbidden answer/trace fields in the question-only artifact. New annotation train, dev, and locked-evaluation roles each remain at zero. The repository now contains 30 leakage-safe input views, 90 coarse/medium/fine `llm_proposed` representations, 90 warning-free deterministic pass records, and three hash-bound review packets. Actual human review records, adjudicated annotations, and corpus records remain at zero.

The current study status is `structural_integrity_complete_human_calibration_pending` and the decision is `UNDECIDED_NEEDS_ANNOTATION_EVIDENCE`. This is not a final vocabulary, gold-corpus, or modeling-ready claim. The next exact task is to collect two independent substantive review sets per granularity—six reviewer-by-granularity files and 180 decisions total—verify their packet/representation hashes, obtain researcher-approved procedural reviewer attestation, and adjudicate every reject, edit, or substantive disagreement.
