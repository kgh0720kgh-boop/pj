...
No human should use the existing granularity HTML packets now. After the A0 raw open-coding instrument and validator are committed and pass their leakage checks, the researcher procedurally signs off two mutually independent, exposure-naive real-human reviewers. This manual sign-off is not machine-authenticatable under the current no-registry contract. Each reviewer independently codes the first committed-order 10-question batch using only the question-only payload, without consultation or external lookup, and submits one immutable raw 10-record artifact.
