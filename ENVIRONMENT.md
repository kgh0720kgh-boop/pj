...
The active sequence is now the v0.2 scale-first question-only AI exploration. Its frozen N=100 input exposes exactly opaque question ID and question text, and excludes environment, answer, trace, grounding, operator proposal, historical graph, other-worker output, and downstream family/metric information. One primary AI record per question is normalized with the frozen deterministic signature contract; the run is descriptive recurrence evidence under that extractor and normalizer, not human evidence or gold data.
...
The next exact research task is to create and freeze a separately versioned cumulative N=300 selection, question-only pool, and exposure manifest with the current 100 as an exact prefix and 200 newly selected eligible questions. Generation and analysis must then reuse the N=100 prompt, schema, semantic-role contract, and deterministic normalizer unchanged. The new committed positions must be distributed across producer partitions instead of assigning one contiguous range to each context, and cumulative analysis must repeat the partition-sensitivity audit. Any extractor or normalizer change starts a new experiment version rather than being mixed into the N=300 curve.

The preserved proposal artifacts and the N=100 scale run truthfully record `model_id=codex_gpt-5`, but the interface exposed no exact model revision and did not support a seed. For the scale run, the partitioned structured JSONL files are the primary capture and no separate raw response exists. The manifests record explicit unavailable/not-supported statuses rather than fabricating values. The structured artifacts and hashes are portable, but exact generation replayability and statistical reviewer independence are not claimed.
