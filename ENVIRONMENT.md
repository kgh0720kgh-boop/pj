...
The next exact research task is to freeze a versioned candidate-backbone/library-and-representative-sampling contract before reading environment contents or execution outcomes. It must retain all 30 contracted families without post-hoc semantic merging, preserve fine/topology/task crosswalks and evidence strength, and deterministically commit frequency/rarity-stratified representative IDs. Only then may environment-aware operator realization begin, with backbone adequacy, grounding, execution, and answer recovery recorded as separate outcomes.
