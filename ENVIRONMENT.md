...
The next exact research task is to freeze a versioned environment input-view and realization protocol for the committed 71-ID order before inspecting row/cell values or linked-document contents for this realization stage. It must bind pinned official-source identities, define a sanitized visibility allowlist, exclude factual answers, official traces, historical graphs, old operator proposals, and grounding/execution outcomes from realization authoring, and avoid treating any preserved candidate vocabulary as final. After that contract is committed, sanitized table/document views may be materialized and representative environment-aware operator realization may begin. Backbone adequacy, environment realization, grounding, execution, and answer recovery remain separate outcomes.
